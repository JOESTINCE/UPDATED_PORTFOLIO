...
const Contact = () => {
  ...
  useEffect(() => {
    ...
    if (titleRef.current) {
      titleRef.current.classList.add('opacity-0', '-translate-y-20', 'rotate-2', 'scale-95', 'transition-all', 'duration-900', 'ease-out', 'will-change-transform');
      observer.observe(titleRef.current);
    }

    if (descRef.current) {
      descRef.current.classList.add('opacity-0', 'translate-y-20', '-rotate-2', 'scale-95', 'transition-all', 'duration-900', 'ease-out', 'delay-300', 'will-change-transform');
      observer.observe(descRef.current);
    }

    // Content and form with gentle effects
    if (contentRef.current) {
      contentRef.current.classList.add('opacity-0', '-translate-x-20', 'translate-y-20', 'rotate-2', 'scale-95', 'transition-all', 'duration-800', 'ease-out', 'delay-600', 'will-change-transform');
      observer.observe(contentRef.current);
    }

    if (formRef.current) {
      formRef.current.classList.add('opacity-0', 'translate-x-20', '-translate-y-20', '-rotate-2', 'scale-95', 'transition-all', 'duration-800', 'ease-out', 'delay-900', 'will-change-transform');
    ...
    <section ref={sectionRef} className="py-20 px-4 bg-gray-900 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div ref={contentRef}>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={index} className="flex items-center gap-4 group hover:scale-102 transition-transform duration-300 cursor-pointer">
                  <div className="w-12 h-12 bg-red-900/50 rounded-full flex items-center justify-center border border-red-600/30 group-hover:border-red-500 group-hover:scale-105 transition-all duration-300">
          ...
          <Card ref={formRef} className="bg-gray-800 border-red-600/30 hover:border-red-500 transition-all duration-500 hover:shadow-2xl hover:shadow-red-600/20 hover:scale-102 group">
            <CardHeader>
              ...
              <form className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="group">
                    ...
                    <Input 
                      id="firstName" 
                      placeholder="John" 
                      className="mt-1 bg-gray-700 border-gray-600 text-white placeholder-gray-400 focus:border-red-500 focus:ring-red-500 hover:border-red-600 transition-all duration-300 focus:scale-102"
                    />
                  </div>
                  <div className="group">
                ...
                <div className="group">
                  ...
                  <textarea 
                    ...
                    className="mt-1 w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 hover:border-red-600 transition-all duration-300 focus:scale-102"
                  />
                </div>
                <Button 
                  type="submit" 
                  className="w-full bg-red-600 hover:bg-red-700 text-white py-3 transition-all duration-500 hover:scale-105 hover:shadow-xl hover:shadow-red-600/50 hover:-translate-y-1"
                >
                  Send Message
                </Button>
              </form>
